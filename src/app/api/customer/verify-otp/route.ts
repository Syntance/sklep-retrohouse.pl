...
export async function POST(request: Request) {
	try {
		...
		if (!parsed.success) {
			return NextResponse.json({ ok: false, error: "Niepoprawny format danych" }, { status: 400 });
		}
		...
		if (!isValid) {
			return NextResponse.json({ ok: false, error: "Niepoprawny kod lub wygasł" }, { status: 401 });
		}
		...
		return NextResponse.json({ ok: false, error: "Błąd weryfikacji" }, { status: 500 });
	}
}
