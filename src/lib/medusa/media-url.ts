...
function rewriteBackendMediaUrlToCdn(absoluteUrl: string): string {
	...
	try {
		...
		const sameHost =
			parsed.hostname === backend.hostname && (parsed.port || "") === (backend.port || "");
...
export function resolveMedusaMediaUrl(url: string | null | undefined): string | undefined {
	...
	try {
		...
		return (
			rewriteLegacyCmsAssetUrl(rewriteBackendMediaUrlToCdn(trimmed)) ??
			rewriteBackendMediaUrlToCdn(trimmed)
		);
	} catch {
		return trimmed;
	}
...
function rewriteLegacyCmsAssetUrl(url: string): string | undefined {
	...
	try {
		...
		if (parsed.pathname.startsWith("/cms-uploads/") || parsed.pathname.startsWith("/cms/")) {
